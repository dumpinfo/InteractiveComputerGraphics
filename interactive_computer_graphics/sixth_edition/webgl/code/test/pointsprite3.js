...
window.onload = function init()
{
  ...
  for(var i = 0; i<numVertices; i++) {
    vertices.push(vec2(2.0*(Math.random() - 0.5), 2.0*(Math.random() - 0.5)));
    angles.push(2.0*(Math.random() - 0.5));
  }
...
function render() {
    ...
    for(var i =0; i<numVertices; i++) {
      vertices[i][0] += 0.01*(Math.random()-0.5);
      vertices[i][1] += 0.01*(Math.random()-0.5);
    ...
    gl.bindBuffer( gl.ARRAY_BUFFER, anglesId );
    gl.bufferSubData( gl.ARRAY_BUFFER, 0, flatten(angles) );
