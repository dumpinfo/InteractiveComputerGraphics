...
for(var i = 0; i<numSprites; i++) {
  ...
  directionArray.push(theta+0.1*Math.random());
}
...
// check for completeness
   ...
   if(status != gl.FRAMEBUFFER_COMPLETE) alert('Frame Buffer Not Complete');
    ...
    gl.bufferData( gl.ARRAY_BUFFER, flatten(directionArray), gl.STATIC_DRAW );

    var vDirection = gl.getAttribLocation( program2, "vDirection" );
    gl.vertexAttribPointer( vDirection, 1, gl.FLOAT, false, 0, 0 );
    gl.enableVertexAttribArray( vDirection );
